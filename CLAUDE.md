...
> (keyless, one XML per Form 4), behind the same persistent **read-through** DB cache + out-of-band
> cron as the news/revenue-segments slices (`POST /internal/insider-transactions/sync`, driven by
> the **weekly** `sync-insider-transactions` workflow — weekly because insider activity is a
> slow-moving conviction signal, and it's the **heaviest** SEC sweep at up to ~26 reads/stock);
> table `stock_insider_transactions` (migration 0028), a time series unique on `stock_id` +
> `accession_number` + `line_index` (the filing id + the transaction's ordinal within it). **One
> deliberate divergence from the sibling cache slices:** the upsert is **insert-only** (a filed
> transaction is a frozen fact — like the rating-changes slice) rather than merge-by-period, and the
> accumulated feed is **pruned** to the newest 100 per stock (like the news feed). Otherwise it's
> the standard read-through + cron shape: a populated symbol is served straight from the DB (no
> TTL), and `SyncInsiderTransactions` walks the anchor stalest-first (`refresh_targets`, un-cached
> first so it also *seeds*) to keep the store warm — so a user request never waits on the
> multi-request filing walk. (This *replaced* the slice's original **TTL-on-read, no-cron** design,
> which self-refreshed on read and so paid the synchronous walk on every stale/cold read — the perf
> issue the cron fixed. The `SqlInvestmentAnalysisCache` TTL precedent it used to cite no longer
> applies.) Best-effort throughout: a stock with no recent Form 4s is a 200 with an empty feed (not
...
└── stocks/                 # the stocks vertical slice
    ...
    │   └── schemas.py           #    HTTP response DTOs (the HTTP endpoints live in endpoints/)
    ├── insider_transactions/ # ─ insider-transactions sub-slice (its OWN entities.py; insert-only cache pruned to newest 100/stock; SEC Form 4; read-through cache + weekly cron):
    ...
    │   ├── insider_transactions_endpoints.py     #  GET /stocks/ticker/{ticker}/insider-transactions (Form 4 buys/sells + net summary, ?open_market_only; read-through cache over SEC, weekly cron)
