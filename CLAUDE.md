...
- `adapters/yfinance_etf_screener_adapter.py` — live source for the ETF slice's bulk screen: **Yahoo via `yfinance`** (`yf.screen` with the *predefined* `top_etfs_us` screen), the curated top-US-ETF set (`ScreenedEtf` per row — AUM + expense ratio) written into the slice's own `etfs` table. Uses the predefined screen because yfinance's custom `FundQuery` exposes no net-assets field, so it can't rank by AUM the way `EquityQuery` ranks stocks by market cap; every row carries `netAssets`, so the read side sorts by AUM. Carries no category — that's the enrichment adapter's job. Maps `PCX` → `NYSEARCA` (the primary ETF venue the stock screen never sees) on top of the shared exchange-code map
- `adapters/yfinance_etf_category_adapter.py` — the ETF slice's per-ticker **category** enrichment (the ETF analogue of `yfinance_classification_adapter`): **Yahoo via `yfinance`** `Ticker.info['category']` (e.g. "Large Growth" → `large_growth`, "Commodities Focused" → `commodities_focused`), keyless, implementing `EtfCategoryProvider`. The bulk screen carries no category (Yahoo publishes it only per-ticker), so the sync fills it a fund at a time; goes through the shared `yfinance_session` crumb-401 retry like the stock classifier
- `stocks/models.py` — the shared `stocks` anchor as its own tiny slice (`app/stocks/stocks/`): owns the `StockRecord` model (the `stocks` table — `ticker` (unique lookup; the column was renamed from `symbol` by migration 0010 — the domain layers still say "symbol"), the fill-once identity facts `name` and `exchange`, and the mutable `revenue_growth_yoy` / `eps_growth_yoy` **latest trailing YoY snapshot** (migration 0011 — percent; EPS on the analyst-consensus/adjusted basis; **overwritten** every refresh by the annual-earnings slice as the newest reported year rolls forward, unlike the fill-once facts), the universe screen facts `sector` / `industry` / `market_cap` / `screened_at` (migration 0012; `industry` added by 0013), the `in_sp500` / `in_nasdaq100` index-membership flags (migration 0014 — `NOT NULL`, default `False`; reconciled by the index-membership slice: current members marked, drop-outs cleared), and the `pe_ratio` trailing-P/E snapshot (migration 0017 — the consensus-basis figure the ticker card computes live, materialized for search sorting; **overwritten** every run by the universe sync's valuation pass, like `market_cap`, and null until four quarters are cached or on a trailing loss)) and its helpers `get_or_create_stock`, `anchor_facts`, `fill_exchange`. Owned by no single feature; per-feature tables hang off it and import it from here
...
└── stocks/                 # the stocks vertical slice
    ...
    │                       #    universe screen + ETF screen: yfinance; index membership: wikipedia)
    ...
    ├── etfs/               # ── ETF sub-slice (owns its OWN `etfs` table — an ETF is not a company; screens the top US ETFs, categorises them, AND reads them back):
    │   ├── entities.py          #    ScreenedEtf (AUM/expense) + EtfClassification (category) + slugify; read-side shapes (EtfSearchCriteria/Result/Page, EtfSort/SortDirection, EtfCategories)
    │   ├── ports.py             #    live-source ports: EtfScreener (bulk screen, no criteria) + EtfCategoryProvider (per-ticker category — the screen carries none)
    │   ├── repository.py        #    abstract persistence ports: EtfRepository (write: upsert + category enrichment) + EtfSearchRepository (read: search + categories)
    │   ├── models.py            #    EtfRecord (the `etfs` table: AUM/expense/category) + get_or_create_etf (standalone anchor, not a stocks child)
    │   ├── db_repository.py     #    SqlEtfRepository (upsert_screen additive + tickers_missing_category/set_category) + SqlEtfSearchRepository (search/categories)
    │   ├── use_cases.py         #    SyncEtfs (write — screen+upsert then per-ticker category enrichment) + SearchEtfs / ListEtfCategories (read)
    │   └── schemas.py           #    HTTP DTOs for the read endpoints (search page + categories menu; endpoints in endpoints/etf_endpoints.py)
    ...
    │   ├── etf_endpoints.py                      #  GET /stocks/etfs (top-ETF search/filter/sort) + GET /stocks/etfs/categories (filter menu)
    │   ├── cron_etf_endpoints.py                 #  POST /internal/etfs/sync (fire-and-forget: screen + category enrichment)
