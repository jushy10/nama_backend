...
The shared kernel (`entities.py`/`ports.py`/`schemas.py`/`wiring.py`/
...
`ports.py` — `company/charts/ports.py` (`CandleProvider`), `market/boards/ports.py` (the two
board providers), `company/logo/ports.py`, and so on. **Exception: the `ai/` slices use the
`interfaces/` layout** — each ABC in its own plain-named file under the slice's
`interfaces/` package (`ai/analysis/interfaces/stock_scorecard_adapter.py` holds
`StockScorecardAdapter`; the package `__init__` re-exports them), with impl files
vendor-prefixed (`db_*`, `bedrock_*`). There the abstract class is the `*Adapter`
(`StockScorecardAdapter`, `AiAnalysisCacheAdapter`, `ConversationModelAdapter`) and the
implementation is the vendor-prefixed `<Vendor><Concern>Adapter`
(`BedrockStockScorecardAdapter`, `DbAiAnalysisCacheAdapter`).

Naming: a live feed is a `*Provider`; static reference data is a `*Repository` (ai
slices excepted — see above).
...
> context is read **DB-only** (via `adapters/db/db_only_context_providers`) so a cache miss
...
Naming: every adapter lives in its vendor's folder as `<concern>_adapter.py` — the folder carries the vendor, the file carries the concern (`adapters/yfinance/quarterly_earnings_adapter.py`, `adapters/alpaca/price_adapter.py`, `adapters/cnn/fear_greed_adapter.py`), even when the vendor has just one file. **Exception: `adapters/bedrock/` files carry the vendor prefix in the file name too** (`bedrock_stock_scorecard_adapter.py`, `bedrock_market_brief_adapter.py`) — the AI slices follow the interfaces/prefixed-impl convention: the abstract class is the plain `*Adapter`, the impl is `<Vendor><Concern>Adapter` — so `BedrockStockScorecardAdapter` (in `bedrock_stock_scorecard_adapter.py`) implements the `StockScorecardAdapter` interface. Outside the ai slices the *class* keeps the name of the **port** it implements — a live-feed port is a `*Provider`, static reference data a `*Repository`.
...
> `adapters/db/annual_earnings_estimates_adapter.py`, which projects the annual-earnings
...
> (`adapters/yfinance/quarterly_earnings_adapter.py`),
...
> (upcoming) (`adapters/yfinance/annual_earnings_adapter.py`), behind the same persistent
...
> cache** via the shared `DbAiAnalysisCacheAdapter` [`kind="ratings"`], DB-only
...
> `adapters/bedrock/bedrock_ratings_analysis_adapter.py`, endpoint in `endpoints/analysis_endpoints.py`), and
...
> (`adapters/yfinance/recommendations_adapter.py`) — this replaced Finnhub's
...
> `adapters/yfinance/rating_changes_adapter.py` reads `Ticker.upgrades_downgrades` (keyless),
...
> **read-through** DB cache as the trends read (`adapters/db/db_cached_rating_changes_adapter.py` —
...
> source is **yfinance (Yahoo)** via `Ticker.news` (`adapters/yfinance/news_adapter.py`),
...
> `adapters/sec_edgar/revenue_segments_adapter.py`, behind the same persistent **read-through** DB
> cache (`adapters/db/db_cached_revenue_segments_adapter.py`) + out-of-band cron
...
> rollup). Live source is **SEC EDGAR** via `adapters/sec_edgar/insider_transactions_adapter.py`
...
> stock (like the news feed); (2) the read is **DB-only** (`adapters/db/db_only_insider_transactions_adapter.py`),
...
> the `DbInvestmentAnalysisCacheAdapter` TTL precedent it used to cite no longer applies. The DB-only read
> mirrors `adapters/db/db_only_context_providers`, the AI-analysis path's no-live-fetch views.) Best-effort
...
> `OptionChainProvider` port (`company/ticker/ports.py` — expirations first, then only the two
> needed expiries) implemented by `adapters/yfinance/options_adapter.py` (Yahoo via
...
```
app/
├── main.py                 # FastAPI app: CORS, lifespan, rate limiting, /healthz, include_router
...
└── stocks/                 # the stocks domain
    ...
    ├── ports.py            # ── SHARED KERNEL ports (StockData/Quote/BulkQuote/Performance/AllTimeHigh/Estimates …)
    ...
    │   └── bedrock/             #    the Claude-on-Bedrock analysers as bedrock_<concern>_adapter.py (stock_scorecard / etf_analysis /
    ...
    │                            #    bedrock_conversation_model_adapter (the agent's model) + cost.py — the ai slices' prefixed-impl convention
