...
> hand-written fake. Never shortcut it by importing a `*_adapter_impl` into a use
> case or an entity.
...
(`interfaces/stock_quote_adapter.py` holds `StockQuoteAdapter`; the package
`__init__` re-exports everything, including support types like each slice's
`RefreshTarget` in `interfaces/types.py`). An implementation is the
`*AdapterImpl` in a `*_adapter_impl.py` file — the vendor/tech is carried by the
folder (`adapters/yfinance/`, `adapters/db/`) or the slice, never the class
name. Persistence abstractions keep the Repository word inside the pattern —
but drop the `Adapter` suffix in slices converged to ARCHITECTURE.md
(`research/agent`, `coverage/news`, `coverage/recommendations`): there the port
is `<Concept>Repository` in `repository.py` and the SQLAlchemy impl is
`Db<Concept>Repository` in `db_repository.py` (`NewsRepository` /
`DbNewsRepository`), with `RefreshTarget` beside the port in `repository.py`;
unconverged slices still use `<X>RepositoryAdapter` /
`<X>RepositoryAdapterImpl` in `<x>_repository_adapter_impl.py`. When one
interface has several impls (the price feeds; the db-cached vs db-only reads),
the classes share the strict `*AdapterImpl` name and the *module* disambiguates —
a co-importing composition root aliases them
(`from ...alpaca.price_adapter_impl import PriceAdapterImpl as AlpacaPriceAdapterImpl`).
...
```python
class GetStockInfo:
    def __init__(self, provider: StockDataAdapter, ...): ...
...
> (`adapters/yfinance/quarterly_earnings_adapter_impl.py`),
...
> (upcoming) (`adapters/yfinance/annual_earnings_adapter_impl.py`), behind the same persistent
...
> `RatingChange`/`AnalystRatingChanges`, and the composite `AnalystInfo`). **Converged to the
> ARCHITECTURE.md canonical slice shape**: `repository.py` (`RecommendationsRepository` +
> `RatingChangesRepository` + `RefreshTarget`) / `db_repository.py` (`DbRecommendationsRepository` +
> `DbRatingChangesRepository`) / `models` / `use_cases` (single public method **`run`**) /
> `api_schemas.py` (DTOs with `from_*` presenter classmethods) / `wiring.py`
> (framework-free `build_get_stock_analyst_info` / `build_sync_recommendations`), with
> `interfaces/` holding only the vendor ports (`RecommendationAdapter`, `RatingChangeAdapter`)
> (both HTTP endpoints live in `app/endpoints/`: the read `analyst_endpoints.py` — a thin
> `get_<action>` Depends shim over the wiring, domain errors translated by the central
> `error_handlers.py` — and the `cron_recommendations_endpoints.py`). The read serves **one consolidated payload** at
...
> (`adapters/yfinance/recommendation_adapter_impl.py`) — this replaced Finnhub's
...
> `adapters/yfinance/rating_change_adapter_impl.py` reads `Ticker.upgrades_downgrades` (keyless),
> keeping the most recent 50 of Yahoo's full multi-year log; `DbRatingChangesRepository` is
...
> **read-through** DB cache as the trends read (`adapters/db/db_cached_rating_change_adapter_impl.py` —
> DB-first, lazy-fill on a cold miss). The `GetStockAnalystInfo` use case reads this leg through
> the `RatingChangeAdapter` port and **swallows its failures** (an empty run, never propagated)
...
> promise the adapter and repository keep, not entity logic). **Converged to the
> ARCHITECTURE.md canonical slice shape**: `repository.py` (`NewsRepository` + `RefreshTarget`) /
> `db_repository.py` (`DbNewsRepository`) / `models` / `use_cases` (single public method
> **`run`**) / `api_schemas.py` (DTOs with `from_*` presenter classmethods) / `wiring.py`
> (framework-free `build_get_stock_news` / `build_sync_stock_news`), with `interfaces/`
> holding only the vendor port (`NewsAdapter`) (both HTTP endpoints live in
> `app/endpoints/`: the read `news_endpoints.py` — a thin `get_<action>` Depends shim over
> the wiring, domain errors translated by the central `error_handlers.py` — and the
...
> rollup). Live source is **SEC EDGAR** via `adapters/sec_edgar/insider_transactions_adapter_impl.py`
...
> stock (like the news feed); (2) the read is **DB-only** (`adapters/db/db_only_insider_transactions_adapter_impl.py`),
...
> the `InvestmentAnalysisCacheAdapterImpl` TTL precedent it used to cite no longer applies. The DB-only read
> mirrors `adapters/db/db_only_context_adapter_impls`, the AI-analysis path's no-live-fetch views.) Best-effort
...
> read-through DB cache through its `QuarterlyEarningsAdapter` port (lazy fill on a cold
...
> everything else through *existing* ports — `StockQuoteAdapter` + `StockPerformanceAdapter`
> + `StockDataAdapter` (the Alpaca singleton, whose missing-keys 503 gate it inherits —
...
result cache uses a **per-kind TTL** (`wiring.analysis_cache_ttl(kind)`), each default tuned to how often that analysis's *input* changes — earnings 12h and ratings/etf 6h (slow DB/cron data, one row per ticker), stock/fundamentals 4h (slow substance + a live-price valuation slice), sector 30m and market 1h (a live intraday board, but one shared row so a long TTL saves ~nothing); override per kind via `ANALYSIS_CACHE_TTL_MINUTES_<KIND>` or pin all at once with the global `ANALYSIS_CACHE_TTL_MINUTES`). On top of the result cache, the **per-symbol** AI reads carry a per-IP **daily generation quota** — its own slice `app/domains/research/rate_limit_quota/` (`models.py` = the `ai_generation_usage` table, migration 0044, plus the query helpers beside the model — the anchor's `get_or_create_stock` pattern; `repository.py` = the abstract `QuotaRepository`; `db_repository.py` = `DbQuotaRepository`, the SQLAlchemy counter composing the model helpers and owning the transaction; `use_cases.py` = `ConsumeGenerationQuota`, the policy: pool + daily limit + UTC day + key truncation). Consumer use cases (the analyses, the ETF analysis, the research agent) take `ConsumeGenerationQuota | None` and call it right before their model call, so it's spent only when a Bedrock generation actually runs — a cache hit is free, and the guard sits after each use case's not-found/no-data checks so a bad symbol never burns one — raising `QuotaExceeded` → **429** via the central handlers when exhausted. Two pools: `AI_ANALYSIS_DAILY_QUOTA` (default 10/day/IP, shared across the stock scorecard / ETF / earnings / ratings / fundamentals analyses) and `AI_RESEARCH_DAILY_QUOTA` (default 5/day/IP, `POST /agents/research` — tighter since every run is several uncached Bedrock calls). The market-wide reads (sector, market summary) are deliberately unmetered — their cache row is shared by all viewers. The counter fails **open** on a DB fault (a broken quota must not take the AI features down; the slowapi per-IP limits stay the hard backstop). The `/internal/*/sync` cron endpoints are guarded
...
2. **Interface** — declare the `*Adapter` ABC the use case needs, one per file in
   the slice's `interfaces/` package (returns entities, raises domain exceptions;
   re-export it from the package `__init__`).
3. **Use case** — write the `execute()` orchestration in the slice's
   `use_cases.py`, depending only on the entity + port.
4. **Adapter impl** — implement the interface against the real vendor/DB as a
   `*AdapterImpl` in `adapters/<vendor>/<concern>_adapter_impl.py` (or the slice's
   `*_repository_adapter_impl.py` for persistence); map vendor models → entities
   and vendor errors → domain exceptions.
