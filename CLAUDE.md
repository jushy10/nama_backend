...
- `adapters/yfinance_etf_screener_adapter.py` — live source for the ETF slice: **Yahoo via `yfinance`** (`yf.screen` with the *predefined* `top_etfs_us` screen), the curated top-US-ETF set (`ScreenedEtf` per row — AUM / expense ratio / YTD return) written into the slice's own `etfs` table. Uses the predefined screen because yfinance's custom `FundQuery` exposes no net-assets field, so it can't rank by AUM the way `EquityQuery` ranks stocks by market cap; every row carries `netAssets`, so the read side sorts by AUM. Maps `PCX` → `NYSEARCA` (the primary ETF venue the stock screen never sees) on top of the shared exchange-code map
...
└── stocks/                 # the stocks vertical slice
    ...
    ├── etfs/               # ── ETF sub-slice (owns its OWN `etfs` table — an ETF is not a company; screens the top US ETFs AND reads them back):
    │   ├── entities.py          #    ScreenedEtf; read-side shapes (EtfSearchCriteria/Result/Page, EtfSort/SortDirection) — no sector/growth/index (simpler than universe)
    │   ├── ports.py             #    live-source port (EtfScreener — no criteria; the predefined screen carries its own)
    │   ├── repository.py        #    abstract persistence ports: EtfRepository (write) + EtfSearchRepository (read)
    │   ├── models.py            #    EtfRecord (the `etfs` table) + get_or_create_etf (standalone anchor, not a stocks child)
    │   ├── db_repository.py     #    SqlEtfRepository (upsert_screen, additive) + SqlEtfSearchRepository (search)
    │   ├── use_cases.py         #    SyncEtfs (write — one screen+upsert, no enrichment) + SearchEtfs (read)
    │   └── schemas.py           #    HTTP DTOs for the read endpoint (search page; endpoint in endpoints/etf_endpoints.py)
    ...
    │   ├── etf_endpoints.py                      #  GET /stocks/etfs (top-ETF search/filter/sort)
    │   ├── cron_etf_endpoints.py                 #  POST /internal/etfs/sync (fire-and-forget)
