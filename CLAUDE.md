...
board providers), `ai/analysis/ports.py` (the five analyser ports + the result
cache), `company/logo/ports.py`, and so on.
...
└── stocks/                 # the stocks domain
    ...
    │   └── bedrock/             #    the Claude-on-Bedrock analysers as <concern>_adapter.py (analysis / etf_analysis / earnings_analysis /
    │                            #    ratings_analysis / fundamentals_analysis / sector_analysis / market_summary / market_brief)
    │                            #    + the screener_query / etf_screener_query translators + research_model_adapter (the agent's model) + cost.py
