...
`NewsRepositoryAdapter` (interface) / `NewsRepositoryAdapterImpl` (the
SQLAlchemy impl, in the slice as `news_repository_adapter_impl.py`). When one
...
> keeping the most recent 50 of Yahoo's full multi-year log; `RatingChangesRepositoryAdapterImpl` is
