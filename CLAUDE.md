...
> the estimates projection). Single-file vendors (alpaca, logodev, wikipedia,
> treasury, cnn, stock_watcher) and the vendor-less `market_routing` composition
> stay at the root as `<vendor>_<concern>_adapter.py`.
...
Naming: a vendor with its own subfolder holds `<concern>_adapter.py` files (`adapters/yfinance/quarterly_earnings_adapter.py`, `adapters/bedrock/analysis_adapter.py`); a root-level single file is `<vendor>_<concern>_adapter.py` (`adapters/alpaca_adapter.py`, `adapters/cnn_fear_greed_adapter.py`). The file *suffix* is only a folder convention; the *class* keeps the name of the **port** it implements — a live-feed port is a `*Provider`, static reference data a `*Repository` — so e.g. `BedrockAnalysisProvider` (in `analysis_adapter.py`) implements the `InvestmentAnalysisProvider` port.
...
└── stocks/                 # the stocks domain
    ...
    ├── adapters/           # ── ALL vendor adapters, grouped by vendor (see the Adapters layer above):
    │   ├── *_adapter.py         #    root singles: alpaca, logodev, wikipedia_index_membership, treasury_yield_curve,
    │   │                        #    cnn_fear_greed, stock_watcher_congress — plus market_routing (the vendor-less price router)
...
tests/                      # offline; fakes through the ports (mirrors app: tests/company/…, tests/market/…, tests/catalog/…, tests/ai/…, tests/adapters, tests/endpoints)
