...
`_impl`-suffixed (`<concern>_adapter_impl.py`). There the abstract class is the
`*Adapter` (`StockScorecardAdapter`, `AiAnalysisCacheAdapter`,
`ConversationModelAdapter`) and the implementation is the `*AdapterImpl`
(`StockScorecardAdapterImpl` in `adapters/bedrock/stock_scorecard_adapter_impl.py`,
`AiAnalysisCacheAdapterImpl` in `ai/analysis/ai_analysis_cache_adapter_impl.py`) —
the vendor is carried by the folder (`adapters/bedrock/`) or the slice, not the
file/class name.
