...
swing-low support levels, and swing-high resistance levels.
"""
...
# --------------------------- Resistance levels (swing-high zones) ---------------------------

# The mirror of support: a resistance level is read off how many separate swing
# highs formed it — a price the market has repeatedly turned *down* from is a
# stickier ceiling than a one-off spike.
_STRONG_MIN_REJECTIONS = 3
_MODERATE_MIN_REJECTIONS = 2


class ResistanceStrength(str, Enum):
    """How firmly a resistance level has held, by the number of swing highs that
    formed it. String values double as the API's JSON values (like RsiSignal)."""

    WEAK = "weak"  # a single swing high
    MODERATE = "moderate"  # two
    STRONG = "strong"  # three or more
...
class ResistanceLevel:
    ...
    repeatedly run into sellers.

    Built by clustering nearby *swing highs* (pivot highs): the level is the mean
    of the highs that formed it. ``touches`` is how many did (the strength
    signal), ``last_touched`` dates the most recent, and ``distance_percent`` is
    how far the level sits above the reference price it was measured against
    (``>= 0`` — resistance is at or above the current price).
    """
...
@dataclass(frozen=True)
class ResistanceLevelSeries:
    """The resistance levels detected for one symbol at one timeframe.

    ``reference_price`` is the latest close the levels were measured against (what
    "above the current price" means here); ``levels`` are strongest-first and can
    be empty when there isn't enough history — or no swing high sits above the
    current price — to find any.
    """
    ...
    timeframe: Timeframe
    reference_price: float
    levels: tuple[ResistanceLevel, ...]


def _rejection_strength_for(touches: int) -> ResistanceStrength:
    """Map a level's swing-high count onto its strength band."""
    if touches >= _STRONG_MIN_REJECTIONS:
        return ResistanceStrength.STRONG
    if touches >= _MODERATE_MIN_REJECTIONS:
        return ResistanceStrength.MODERATE
    return ResistanceStrength.WEAK


def _pivot_high_indices(highs: Sequence[float], window: int) -> list[int]:
    """Indices of the swing highs in ``highs`` — each a bar whose high is at or
    above every high within ``window`` bars on both sides (a *pivot high* /
    fractal). The mirror of ``_pivot_low_indices``.

    The first and last ``window`` bars are skipped: a swing high isn't confirmed
    until ``window`` bars have printed on each side, so the edges can't form one.
    A flat peak (consecutive equal highs) collapses to its first bar, so one
    inverted-V turn counts as a single touch rather than several.
    """
    n = len(highs)
    pivots: list[int] = []
    for i in range(window, n - window):
        ...
        if high < max(highs[i - window : i + window + 1]):
            continue
        # A flat top flags every equal bar in the run; keep only the first.
        if pivots and i - pivots[-1] <= window and highs[pivots[-1]] == high:
            continue
        pivots.append(i)
    return pivots
...
def compute_resistance_levels(
    ...
    *,
    ...
    """Detect horizontal resistance levels from a chronological (oldest-first)
    high series and the timestamps those highs fall on. The mirror of
    ``compute_support_levels``.

    Swing highs (``_pivot_high_indices``) are clustered by price — a high within
    ``tolerance`` (a fraction, e.g. ``0.02`` = 2%) below a cluster's peak high
    joins it — and each cluster becomes a level at the mean of its highs. Only
    levels at or above ``reference_price`` (the latest close) are kept: resistance
    sits over the current price. Levels are ranked by strength (touch count, then
    recency), the top ``max_levels`` are taken, and they're returned nearest-first
    (lowest price first — just over the quote).

    Returns ``[]`` when there isn't enough history (fewer than ``2 * window + 1``
    highs), when no swing high sits at or above the price, or when
    ``reference_price`` is non-positive — "couldn't find any" is not an error.

    Raises:
        ValueError: ``window < 2``, ``tolerance`` outside ``(0, 1)``,
            ``max_levels < 1``, or ``highs`` and ``timestamps`` differ in length.
    """
    if window < 2:
        raise ValueError("window must be at least 2.")
    if not 0.0 < tolerance < 1.0:
    ...
    if max_levels < 1:
        raise ValueError("max_levels must be at least 1.")
    if len(highs) != len(timestamps):
        raise ValueError("highs and timestamps must be the same length.")

    n = len(highs)
    if reference_price <= 0 or n < 2 * window + 1:
        return []

    pivots = _pivot_high_indices(highs, window)
    if not pivots:
        return []
    ...
    # Agglomerate swing highs into zones: walk them in descending price order and
    # keep extending a cluster while the next high is within `tolerance` of the
    # cluster's peak (highest) high — anchoring to the peak bounds each zone's
    # width so a gentle down-drift can't chain unrelated highs into one runaway
    # level.
    members = sorted(
        ((highs[i], timestamps[i]) for i in pivots), key=lambda m: m[0], reverse=True
    )
    clusters: list[list[tuple[float, datetime]]] = []
    for price, ts in members:
        if clusters and (clusters[-1][0][0] - price) / clusters[-1][0][0] <= tolerance:
            clusters[-1].append((price, ts))
        else:
            clusters.append([(price, ts)])

    ranked: list[tuple[float, ResistanceLevel]] = []
    for cluster in clusters:
        price = round(sum(p for p, _ in cluster) / len(cluster), 2)
        if price < reference_price:  # at/below the quote — not resistance
            continue
        touches = len(cluster)
        latest_ts = max(ts for _, ts in cluster)
        recency = (latest_ts - span_start).total_seconds() / span_seconds  # 0..1
        level = ResistanceLevel(
            ...
            touches=touches,
            last_touched=latest_ts.date(),
            strength=_rejection_strength_for(touches),
            distance_percent=round((price - reference_price) / reference_price * 100, 2),
        )
        # Touch count dominates; recency (0..1) breaks ties toward fresher levels.
        ranked.append((touches + recency, level))
    ...
    # Strongest first (nearest the price on a tie), keep the top N, then present
    # them nearest-resistance-first — the lowest price, just over the quote. The
    # tie key negates distance so "nearest" means the *smaller* gap above price.
    ranked.sort(key=lambda item: (item[0], -item[1].distance_percent), reverse=True)
    top = [level for _, level in ranked[:max_levels]]
    top.sort(key=lambda level: level.price)
    return top
...
def resistance_levels(
    ...
    *,
    ...
    """Detect resistance levels for a candle series, measured against its latest
    close. The mirror of ``support_levels``.

    Pure: the detection (``compute_resistance_levels``) runs on the candles' highs
    and their timestamps, with the final close as the reference price the levels
    sit above. Given the same series it always returns the same result.
    """
    highs = [candle.high for candle in series.candles]
    timestamps = [candle.timestamp for candle in series.candles]
    reference_price = series.candles[-1].close if series.candles else 0.0
