...
Every AI-generated read the API serves, plus ``GetStockInfo`` — the enriched
stock snapshot that is the per-stock analysis's context (the standalone
``GET /stocks/{symbol}`` endpoint was removed; the snapshot now exists to feed
the analyser). Each use case gathers primary data plus best-effort context
through ports, then hands the lot to an injected analyser; the analyser reasons
only over what it's handed and fetches nothing itself.
"""
