"""HTTP API for invoking the news refresh — the cron entrypoint.

The refresh is a use case (``SyncStockNews``) driven over HTTP: a scheduler (the sync-news
...
Wiring lives here, the composition-root way: ``run_news_sync`` opens a fresh session and
builds the live yfinance adapter + the SQL repository for the use case. yfinance reads Yahoo's
public data with no API key, so there's no credential to gate on; the sync is always
...
from app.stocks.news.use_cases import NewsSyncReport, SyncStockNews
