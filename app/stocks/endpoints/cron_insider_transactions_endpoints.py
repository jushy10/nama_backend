"""HTTP API for invoking the insider-transactions refresh — the cron entrypoint.

The refresh is a use case (``SyncInsiderTransactions``) driven over HTTP: a scheduler (the
sync-insider-transactions GitHub workflow, or any cron) POSTs here to kick it off.

The sweep is **fire-and-forget**. Walking a stock's recent Form 4 filings (one XML per filing)
across the anchor takes a long time, but the API Gateway in front of the app has a hard 30s
integration timeout — a synchronous run would 504 at the gateway while the app kept working. So
the endpoint schedules the sweep on a background thread and returns ``202`` at once; the shared
``background_sync`` helper owns the threading, the single-flight guard, and the exception handling
(see it for the full rationale and the per-process-guard caveat). A partial run is safe:
``upsert`` commits per stock and the sweep is stalest-first, so an interrupted run just resumes on
the next trigger.

Wiring lives here, the composition-root way: ``run_insider_transactions_sync`` opens a fresh
session and builds the live SEC EDGAR adapter + the SQL repository for the use case. EDGAR reads
SEC's public filings with no API key, so there's no credential to gate on; the sync is always
constructable. The adapter is paced so the serial sweep stays under EDGAR's ~10 req/s ceiling.
...
from app.stocks.insider_transactions.use_cases import (
