"""HTTP API for invoking the revenue-segments refresh — the cron entrypoint.

The refresh is a use case (``SyncRevenueSegments``) driven over HTTP: a scheduler (the
sync-revenue-segments GitHub workflow, or any cron) POSTs here to kick it off.

The sweep is **fire-and-forget**. Walking a filing per stock across the anchor takes minutes,
but the API Gateway in front of the app has a hard 30s integration timeout — a synchronous run
would 504 at the gateway while the app kept working. So the endpoint schedules the sweep on a
...
Wiring lives here, the composition-root way: ``run_revenue_segments_sync`` opens a fresh session
and builds the live SEC EDGAR adapter + the SQL repository for the use case. EDGAR reads SEC's
public filings with no API key, so there's no credential to gate on; the sync is always
...
from app.stocks.revenue_segments.use_cases import (
