...
# early on a sweep often succeeds a pass later — recovering it here beats waiting a *month* for
# the next annual run. Genuine no-coverage (an empty timeline / ``StockNotFound``) is never
...
class GetAnnualEarnings:
    ...
    def run(self, symbol: str) -> AnnualEarningsTimeline:
        return self._provider.get_annual_earnings(normalize_symbol(symbol))
...
class SyncAnnualEarnings:
    ...
    def run(self, *, limit: int | None = None) -> AnnualEarningsSyncReport:
        ...
        for attempt in range(self._max_attempts):
            label = (
                "annual-earnings sync"
                if attempt == 0
                else f"annual-earnings sync (retry {attempt})"
            )
    ...
    def _run_pass(self, targets: list[RefreshTarget], *, label: str) -> _PassOutcome:
        ...
        try:
            futures = [
                pool.submit(self._provider.get_annual_earnings, target.symbol)
            ...
            for target, future in iter_with_progress(
                ...
                # delete every year. Skip it and count a failure; it's coverage-shaped rather
