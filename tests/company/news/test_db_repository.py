...
from app.stocks.company.news.db_repository import (
    _MAX_STORED_ARTICLES,
    SqlNewsRepository,
)
from app.stocks.company.news.entities import NewsArticle, StockNews
from app.stocks.company.news.models import (
    StockNewsRecord,
    StockRecord,
    get_or_create_stock,
)
...
def repo(session) -> SqlNewsRepository:
    return SqlNewsRepository(session, now=lambda: _NOW)
...
def test_refresh_targets_orders_by_last_refresh_not_oldest_row(session):
    ...
    ancient = SqlNewsRepository(session, now=lambda: _NOW - timedelta(days=90))
    mid = SqlNewsRepository(session, now=lambda: _NOW - timedelta(days=10))
    fresh = SqlNewsRepository(session, now=lambda: _NOW)
