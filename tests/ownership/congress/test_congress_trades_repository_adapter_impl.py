...
from app.domains.ownership.congress import models
from app.domains.ownership.congress.congress_trades_repository_adapter_impl import (
    CongressTradesRepositoryAdapterImpl,
    _MAX_STORED_TRADES,
)
from app.domains.ownership.congress.entities import CongressActivity, CongressTrade
from app.domains.ownership.congress.models import StockCongressTradeRecord, StockRecord
...
def repo(session, *, now=None) -> CongressTradesRepositoryAdapterImpl:
    return CongressTradesRepositoryAdapterImpl(session, now=now or (lambda: _NOW))
...
def test_prune_is_enforced_under_production_autoflush_false():
    ...
    with prod_session() as db:
        r = CongressTradesRepositoryAdapterImpl(db, now=lambda: _NOW)
        total = _MAX_STORED_TRADES + 5
        base = date(2020, 1, 1)
        trades = [
            _trade(member=f"Member {i:04d}", disc_date=base + timedelta(days=i))
