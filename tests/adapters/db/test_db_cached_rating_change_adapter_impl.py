...
def test_hit_serves_stored_events_without_touching_the_live_source():
    stored = _a_run("AAPL")
    inner = _FakeInner()
    out = RatingChangeAdapterImpl(inner, _FakeRepo(stored=stored)).get_rating_changes(
        "AAPL"
    )
...
def test_miss_fetches_once_stores_and_returns():
    ...
    out = RatingChangeAdapterImpl(inner, repo).get_rating_changes("AAPL")
    assert out is live
...
def test_empty_live_result_is_returned_but_not_cached():
    ...
    repo = _FakeRepo(stored=None)
    out = RatingChangeAdapterImpl(inner, repo).get_rating_changes("ZZZZ")
    assert out.is_empty
...
def test_live_failure_on_a_miss_propagates():
    ...
    with pytest.raises(StockDataUnavailable):
        RatingChangeAdapterImpl(inner, _FakeRepo(stored=None)).get_rating_changes(
            "AAPL"
        )
