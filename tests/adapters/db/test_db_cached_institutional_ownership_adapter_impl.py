...
def test_hit_serves_stored_rows_without_touching_the_live_source():
    ...
    inner = _FakeInner()
    out = InstitutionalOwnershipAdapterImpl(
...
def test_miss_fetches_once_stores_and_returns():
    ...
    out = InstitutionalOwnershipAdapterImpl(inner, repo).get_institutional_ownership("AAPL")
    assert out is live
...
def test_empty_live_result_is_returned_but_not_cached():
    ...
    repo = _FakeRepo(stored=None)
    out = InstitutionalOwnershipAdapterImpl(inner, repo).get_institutional_ownership("ZZZZ")
    assert out.is_empty
...
def test_live_failure_on_a_miss_propagates():
    ...
    with pytest.raises(StockDataUnavailable):
        InstitutionalOwnershipAdapterImpl(inner, _FakeRepo(stored=None)).get_institutional_ownership("AAPL")
