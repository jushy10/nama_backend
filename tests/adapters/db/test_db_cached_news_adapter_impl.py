...
from app.stocks.adapters.db.db_cached_news_adapter_impl import NewsAdapterImpl
...
def test_hit_serves_stored_rows_without_touching_the_live_source():
    stored = _a_run("AAPL")
    inner = _FakeInner()
    out = NewsAdapterImpl(inner, _FakeRepo(stored=stored)).get_news("AAPL")
...
def test_miss_fetches_once_stores_and_returns():
    ...
    out = NewsAdapterImpl(inner, repo).get_news("AAPL")
    assert out is live
...
def test_empty_live_result_is_returned_but_not_cached():
    ...
    repo = _FakeRepo(stored=None)
    out = NewsAdapterImpl(inner, repo).get_news("ZZZZ")
    assert out.is_empty
...
def test_live_failure_on_a_miss_propagates():
    ...
    with pytest.raises(StockDataUnavailable):
        NewsAdapterImpl(inner, _FakeRepo(stored=None)).get_news("AAPL")
