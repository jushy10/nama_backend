...
def test_quarterly_serves_stored_timeline():
    stored = _quarterly()
    provider = QuarterlyEarningsAdapterImpl(_FakeRepo(stored))
...
def test_quarterly_miss_yields_empty_not_a_fetch():
    ...
    result = QuarterlyEarningsAdapterImpl(repo).get_quarterly_earnings("AAPL")
...
def test_quarterly_read_error_degrades_to_empty():
    provider = QuarterlyEarningsAdapterImpl(_FakeRepo(raises=RuntimeError("db down")))
    assert provider.get_quarterly_earnings("AAPL").is_empty


def test_annual_serves_stored_and_misses_to_empty():
    stored = _annual()
    assert AnnualEarningsAdapterImpl(_FakeRepo(stored)).get_annual_earnings("AAPL") is stored
    miss = AnnualEarningsAdapterImpl(_FakeRepo(None)).get_annual_earnings("AAPL")
    assert miss.is_empty and miss.symbol == "AAPL"


def test_annual_read_error_degrades_to_empty():
    provider = AnnualEarningsAdapterImpl(_FakeRepo(raises=RuntimeError("boom")))
...
def test_recommendations_serves_stored_and_misses_to_empty():
    ...
    assert (
        RecommendationAdapterImpl(_FakeRepo(stored)).get_recommendations("AAPL")
        is stored
    )
    miss = RecommendationAdapterImpl(_FakeRepo(None)).get_recommendations("AAPL")
    assert miss.is_empty and miss.symbol == "AAPL"


def test_recommendations_read_error_degrades_to_empty():
    provider = RecommendationAdapterImpl(_FakeRepo(raises=RuntimeError("boom")))
