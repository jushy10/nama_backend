...
def test_projects_first_two_upcoming_years_as_fy1_and_fy2():
    ...
    est = AnalystEstimatesAdapterImpl(FakeRepo(timeline)).get_estimates("AAPL")
...
def test_uncached_symbol_yields_empty_block():
    est = AnalystEstimatesAdapterImpl(FakeRepo(None)).get_estimates("ZZZZ")
    assert est.is_empty
...
def test_storage_failure_becomes_domain_error():
    provider = AnalystEstimatesAdapterImpl(FakeRepo(fail=True))
