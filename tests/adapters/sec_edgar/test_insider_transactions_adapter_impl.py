...
def _provider(http) -> InsiderTransactionsAdapterImpl:
    p = InsiderTransactionsAdapterImpl()
    p._http = http
    return p
...
def test_live_order_matches_db_serving_order():
    ...
    with Session(engine) as db:
        InsiderTransactionsRepositoryAdapterImpl(db, now=lambda: now).upsert("AAPL", "Apple", live)
        stored = InsiderTransactionsRepositoryAdapterImpl(db).get("AAPL")
