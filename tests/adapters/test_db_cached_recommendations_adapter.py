...
def test_hit_serves_stored_rows_without_touching_the_live_source():
    stored = _a_run("AAPL")
    inner = _FakeInner()
    out = DbCachedRecommendationProvider(inner, _FakeRepo(stored=stored)).get_recommendations("AAPL")
...
def test_miss_fetches_once_stores_and_returns():
    ...
    out = DbCachedRecommendationProvider(inner, repo).get_recommendations("AAPL")
    assert out is live
...
def test_empty_live_result_is_returned_but_not_cached():
    ...
    repo = _FakeRepo(stored=None)
    out = DbCachedRecommendationProvider(inner, repo).get_recommendations("ZZZZ")
    assert out.is_empty
...
def test_live_failure_on_a_miss_propagates():
    ...
    with pytest.raises(StockDataUnavailable):
        DbCachedRecommendationProvider(inner, _FakeRepo(stored=None)).get_recommendations("AAPL")
