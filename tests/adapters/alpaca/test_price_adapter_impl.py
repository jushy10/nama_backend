...
def provider_with(data_client, trading_client) -> PriceAdapterImpl:
    ...
    p = PriceAdapterImpl("dummy-key", "dummy-secret")
    p._data = data_client
...
def test_to_entity_maps_every_field():
    stock = PriceAdapterImpl._to_entity(
...
def test_to_quote_maps_snapshot_fields():
    quote = PriceAdapterImpl._to_quote("AAPL", make_snapshot())
...
def test_compute_performance_maps_each_window():
    ...
    perf = PriceAdapterImpl._compute_performance(
...
def test_compute_performance_empty_bars_all_none():
    assert PriceAdapterImpl._compute_performance([]) == StockPerformance(
...
def test_compute_performance_insufficient_history_yields_none():
    ...
    perf = PriceAdapterImpl._compute_performance(bars)
    assert perf.one_week == 10.0
...
def bars_provider(client) -> PriceAdapterImpl:
    p = PriceAdapterImpl("dummy-key", "dummy-secret")
    p._data = client
    return p
...
def test_to_candle_maps_fields_and_casts_volume():
    ...
    candle = PriceAdapterImpl._to_candle(bar)
...
def test_to_all_time_high_picks_peak_high_and_bounds():
    high = PriceAdapterImpl._to_all_time_high(ath_bars())
...
def test_provider_implements_all_ports():
    ...
    # isinstance check to reuse the provider as the StockPerformanceAdapter, so
    # a dropped base class would silently stop a feature from populating.
    p = PriceAdapterImpl("dummy-key", "dummy-secret")
    assert isinstance(p, StockDataAdapter)
    assert isinstance(p, StockQuoteAdapter)
    assert isinstance(p, BulkQuoteAdapter)
    assert isinstance(p, StockPerformanceAdapter)
    assert isinstance(p, AllTimeHighAdapter)
    assert isinstance(p, CandleAdapter)
    assert isinstance(p, SectorPerformanceAdapter)
    assert isinstance(p, MarketOverviewAdapter)
...
def test_to_alpaca_symbol_maps_share_class_dash_to_dot():
    ...
    # through untouched.
    assert PriceAdapterImpl._to_alpaca_symbol("BRK-B") == "BRK.B"
    assert PriceAdapterImpl._to_alpaca_symbol("BF-B") == "BF.B"
    assert PriceAdapterImpl._to_alpaca_symbol("AAPL") == "AAPL"
