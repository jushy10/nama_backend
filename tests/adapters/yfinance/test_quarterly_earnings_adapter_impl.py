...
def provider_with(ticker: FakeTicker) -> QuarterlyEarningsAdapterImpl:
    return QuarterlyEarningsAdapterImpl(ticker_factory=lambda _symbol: ticker)
...
def provider_with_currency(
    ...
    return QuarterlyEarningsAdapterImpl(ticker_factory=factory)
...
def test_domestic_issuer_makes_no_fx_call():
    ...
    tl = QuarterlyEarningsAdapterImpl(ticker_factory=factory).get_quarterly_earnings(
        "AAPL"
    )
    by_quarter = {(q.fiscal_year, q.fiscal_quarter): q for q in tl.past}
