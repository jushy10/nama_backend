...
def provider_with(frame=None, error=None) -> RatingChangeAdapterImpl:
    return RatingChangeAdapterImpl(
...
def test_caps_the_stored_window_at_the_most_recent():
    ...
    assert len(changes.changes) == RatingChangeAdapterImpl._MAX_CHANGES
...
def test_ticker_construction_failure_raises_unavailable():
    ...
    provider = RatingChangeAdapterImpl(ticker_factory=_boom)
    with pytest.raises(StockDataUnavailable):
