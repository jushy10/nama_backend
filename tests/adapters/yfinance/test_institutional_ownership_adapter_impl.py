...
def _provider(**kw) -> InstitutionalOwnershipAdapterImpl:
    return InstitutionalOwnershipAdapterImpl(
...
def test_ticker_construction_failure_raises_unavailable():
    ...
    provider = InstitutionalOwnershipAdapterImpl(ticker_factory=_boom)
    with pytest.raises(StockDataUnavailable):
