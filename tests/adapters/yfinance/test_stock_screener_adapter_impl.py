...
def provider(pages=None, **kw) -> StockScreenerAdapterImpl:
    return StockScreenerAdapterImpl(screen_page=FakePages(pages, **kw))
...
def test_paginates_until_total_reached():
    ...
    out = StockScreenerAdapterImpl(screen_page=pages).screen(
...
def test_passes_the_floor_through_to_the_fetch():
    ...
    StockScreenerAdapterImpl(screen_page=pages).screen(min_market_cap=5_000_000_000)
...
def test_ca_pass_forwards_the_region_to_the_fetch():
    ...
    StockScreenerAdapterImpl(screen_page=pages).screen(
