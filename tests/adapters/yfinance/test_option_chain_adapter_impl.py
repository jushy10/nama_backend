...
def _provider(ticker: _FakeTicker) -> OptionChainAdapterImpl:
    return OptionChainAdapterImpl(ticker_factory=lambda symbol: ticker)
...
def test_ticker_construction_failure_raises_unavailable():
    ...
    provider = OptionChainAdapterImpl(ticker_factory=_boom)
    with pytest.raises(StockDataUnavailable):
