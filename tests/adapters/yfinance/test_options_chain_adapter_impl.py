...
from app.adapters.yfinance.options_chain_adapter_impl import (
    OptionsChainAdapterImpl,
)
from app.domains.shared.exceptions import StockDataUnavailable
from app.domains.pricing.options.entities import OptionType
...
def _provider(ticker: _FakeTicker) -> OptionsChainAdapterImpl:
    return OptionsChainAdapterImpl(ticker_factory=lambda symbol: ticker)
...
def test_ticker_construction_failure_raises_unavailable():
    ...
    provider = OptionsChainAdapterImpl(ticker_factory=_boom)
    with pytest.raises(StockDataUnavailable):
