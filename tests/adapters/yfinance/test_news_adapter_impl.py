...
def provider_with(news=None, error=None) -> NewsAdapterImpl:
    return NewsAdapterImpl(ticker_factory=lambda symbol: _FakeTicker(news, error))
...
def test_ticker_construction_failure_raises_unavailable():
    ...
    provider = NewsAdapterImpl(ticker_factory=_boom)
    with pytest.raises(StockDataUnavailable):
