...
def test_parses_tool_call_into_entity():
    ...
    provider = EtfAnalysisAdapterImpl(client=client, model_id="test-model")
...
def test_renders_the_fund_facts_into_the_prompt():
    client = _StubClient(_tool_message())
    EtfAnalysisAdapterImpl(client=client).analyze(
...
def test_renders_holdings_sectors_and_description_blocks():
    client = _StubClient(_tool_message())
    EtfAnalysisAdapterImpl(client=client).analyze(_detail())
...
def test_maps_a_client_error_to_a_domain_error():
    with pytest.raises(StockDataUnavailable):
        EtfAnalysisAdapterImpl(client=_BoomClient()).analyze(_detail())
...
def test_retries_once_when_bullets_come_back_empty():
    ...
    analysis = EtfAnalysisAdapterImpl(client=client).analyze(_detail())
...
def test_incomplete_retry_escalates_to_the_recovery_model():
    ...
    EtfAnalysisAdapterImpl(client=client, recovery_model_id=recovery).analyze(
        _detail()
    )

    assert len(client.calls) == 2
    assert client.calls[0]["model"] == EtfAnalysisAdapterImpl._DEFAULT_MODEL_ID
...
def test_escalated_recovery_failure_is_best_effort():
    ...
    analysis = EtfAnalysisAdapterImpl(
