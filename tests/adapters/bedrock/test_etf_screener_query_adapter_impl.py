...
def _translator(message) -> EtfScreenerQueryAdapterImpl:
    return EtfScreenerQueryAdapterImpl(client=_StubClient(message))
...
def test_constrains_categories_to_the_supplied_vocabulary():
    ...
    EtfScreenerQueryAdapterImpl(client=client).translate(
...
def test_omits_the_category_field_when_the_vocabulary_is_empty():
    ...
    EtfScreenerQueryAdapterImpl(client=client).translate("x", categories=[])
...
def test_vendor_failure_becomes_stock_data_unavailable():
    translator = EtfScreenerQueryAdapterImpl(client=_BoomClient())
