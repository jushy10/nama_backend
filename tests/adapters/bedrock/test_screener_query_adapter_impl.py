...
def _translator(message) -> ScreenerQueryAdapterImpl:
    return ScreenerQueryAdapterImpl(client=_StubClient(message))
...
def test_constrains_sector_and_industry_to_the_supplied_vocabulary():
    ...
    ScreenerQueryAdapterImpl(client=client).translate(
...
def test_vendor_failure_becomes_stock_data_unavailable():
    translator = ScreenerQueryAdapterImpl(client=_BoomClient())
