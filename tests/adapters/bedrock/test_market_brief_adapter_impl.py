...
def test_parses_tool_call_into_entity():
    ...
    provider = MarketBriefAdapterImpl(client=client, model_id="test-model")
...
def test_renders_context_into_the_prompt():
    client = _StubClient(_tool_message())
    MarketBriefAdapterImpl(client=client).generate(_context(), _D)
...
def test_retries_when_sections_come_back_empty():
    ...
    brief = MarketBriefAdapterImpl(client=client).generate(_context(), _D)
...
def test_maps_a_client_error_to_a_domain_error():
    with pytest.raises(StockDataUnavailable):
        MarketBriefAdapterImpl(client=_BoomClient()).generate(_context(), _D)
