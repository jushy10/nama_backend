...
from app.domains.listings.fundamentals.fundamentals_repository_adapter_impl import FundamentalsRepositoryAdapterImpl
from app.domains.listings.fundamentals.entities import Fundamentals
from app.domains.listings.fundamentals.interfaces import RefreshTarget
...
def test_upsert_lands_every_figure_and_stamps_the_sync_time(session):
    FundamentalsRepositoryAdapterImpl(session, now=_at(day=4)).upsert("AAPL", "Apple Inc.", _full())
...
def test_upsert_overwrites_a_stale_figure_to_none(session):
    repo = FundamentalsRepositoryAdapterImpl(session, now=_at(day=4))
    ...
    repo2 = FundamentalsRepositoryAdapterImpl(session, now=_at(day=11))
...
def test_upsert_never_clobbers_a_known_name_with_none(session):
    ...
    session.commit()
    FundamentalsRepositoryAdapterImpl(session).upsert("AAPL", None, Fundamentals(net_margin=25.0))
...
def test_refresh_targets_orders_unsynced_first_then_stalest(session):
    ...
    FundamentalsRepositoryAdapterImpl(session, now=_at(day=2)).upsert("OLD", None, Fundamentals(beta=1.0))
    FundamentalsRepositoryAdapterImpl(session, now=_at(day=9)).upsert("NEW", None, Fundamentals(beta=1.0))
    get_or_create_stock(session, "FRESH", "Fresh Co.")  # never synced -> NULL stamp
    session.commit()

    targets = FundamentalsRepositoryAdapterImpl(session).refresh_targets(None)
...
def test_refresh_targets_honours_the_cap(session):
    ...
    targets = FundamentalsRepositoryAdapterImpl(session).refresh_targets(2)
