...
from app.domains.listings.index_membership.index_membership_repository_adapter_impl import IndexMembershipRepositoryAdapterImpl
...
def test_reconcile_marks_members_and_creates_absent_anchors(session):
    counts = IndexMembershipRepositoryAdapterImpl(session).reconcile(
...
def test_reconcile_clears_dropouts(session):
    repo = IndexMembershipRepositoryAdapterImpl(session)
...
def test_reconcile_does_not_clobber_existing_anchor_facts(session):
    ...
    IndexMembershipRepositoryAdapterImpl(session).reconcile(
        _snap(sp500=("AAPL",)), sync_sp500=True, sync_nasdaq100=False
    )
