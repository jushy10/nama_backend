...
def test_tickers_missing_industry_lists_unclassified_by_market_cap_and_capped(session):
    r = repo(session)
    r.upsert_screen(
        (
    ...
    assert r.tickers_missing_industry(10) == ("AAPL", "XOM", "TSLA")
    assert r.tickers_missing_industry(2) == ("AAPL", "XOM")
