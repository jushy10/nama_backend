...
from app.stocks.catalog.universe.repository_adapter_impl import (
    StockSearchRepositoryAdapterImpl,
    UniverseRepositoryAdapterImpl,
)
...
def repo(session, *, now=_NOW) -> UniverseRepositoryAdapterImpl:
    return UniverseRepositoryAdapterImpl(session, now=lambda: now)
...
# --- StockSearchRepositoryAdapterImpl (the read side) ----------------------------------------------
...
def test_search_maps_every_row_field(session):
    ...
    (result,) = StockSearchRepositoryAdapterImpl(session).search(_criteria()).results
...
def test_pe_ratios_for_industry_empty_for_an_unknown_industry(session):
    _seed(session, "NVDA", industry="semiconductors", pe_ratio=46.5)
    assert StockSearchRepositoryAdapterImpl(session).pe_ratios_for_industry("nonesuch") == ()


def test_industry_for_ticker_returns_the_stored_slug(session):
    _seed(session, "NVDA", industry="semiconductors", pe_ratio=46.5)
    assert (
        StockSearchRepositoryAdapterImpl(session).industry_for_ticker("NVDA")
        == "semiconductors"
    )
...
def test_anchor_metrics_for_ticker_returns_the_stored_figures(session):
    ...
    metrics = StockSearchRepositoryAdapterImpl(session).anchor_metrics_for_ticker("NVDA")
