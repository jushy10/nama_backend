...
Offline: a fake GetQuarterlyEarnings is injected through dependency_overrides, so this
checks only the controller + presenter — the JSON shape and counts, an empty timeline as a
200 (not a 404), and bad input as a 400 — without touching Yahoo or the database.
"""
...
def _client_with(fake: FakeUseCase) -> TestClient:
    app = FastAPI()
...
def test_presents_the_timeline_with_counts():
    fake = FakeUseCase(result=_timeline())
    resp = _client_with(fake).get("/stocks/AAPL/earnings/quarterly")
...
def test_empty_timeline_is_a_200_with_no_quarters():
    fake = FakeUseCase(result=QuarterlyEarningsTimeline("ZZZZ", ()))
    resp = _client_with(fake).get("/stocks/ZZZZ/earnings/quarterly")
    assert resp.status_code == 200
    body = resp.json()
...
def test_bad_symbol_is_a_400():
    fake = FakeUseCase(error=ValueError("'123' is not a valid stock symbol."))
    resp = _client_with(fake).get("/stocks/123/earnings/quarterly")
