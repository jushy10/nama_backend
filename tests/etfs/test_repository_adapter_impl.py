...
from app.domains.etfs.models import (
    EtfRecord,
...
def repo(session, *, now=_NOW) -> EtfRepositoryAdapterImpl:
    return EtfRepositoryAdapterImpl(session, now=lambda: now)
...
def test_profile_refresh_targets_orders_never_fetched_then_stalest(session):
    ...
    EtfRepositoryAdapterImpl(session, now=lambda: _NOW).upsert_profile("ARKK", EtfProfile(nav=1.0))
    EtfRepositoryAdapterImpl(session, now=lambda: _NOW + timedelta(hours=1)).upsert_profile(
...
# --- EtfSearchRepositoryAdapterImpl (the read side) ------------------------------------------------
...
def test_search_matches_name_or_ticker_substring_case_insensitively(session):
    ...
    r = EtfSearchRepositoryAdapterImpl(session)
...
def test_search_maps_every_row_field(session):
    ...
    (result,) = EtfSearchRepositoryAdapterImpl(session).search(_criteria()).results
...
def test_get_stored_profile_reads_scalars_and_children(session):
    ...
    EtfRepositoryAdapterImpl(session, now=lambda: _NOW).upsert_profile("VOO", _full_profile())

    profile = EtfLookupRepositoryAdapterImpl(session).get_stored_profile("VOO")
