...
from app.stocks.endpoints import cron_news_endpoints as cron
from app.stocks.news.use_cases import NewsSyncReport
...
class _FakeRunner:
    ...
    def __call__(self, limit: int | None = None) -> NewsSyncReport:
...
def test_accepts_the_trigger_and_runs_the_sweep_with_the_limit():
    fake = _FakeRunner(NewsSyncReport(refreshed=7, failed=2, limit=50))
    resp = _client(fake).post("/internal/news/sync?limit=50")
...
def test_defaults_to_unlimited_when_omitted():
    fake = _FakeRunner(NewsSyncReport(refreshed=0, failed=0, limit=None))
    resp = _client(fake).post("/internal/news/sync")
    assert resp.status_code == 202
...
def test_a_trigger_while_a_sweep_runs_is_a_noop():
    fake = _FakeRunner(NewsSyncReport(0, 0, 1))
...
def test_rejects_an_out_of_range_limit():
    ...
    assert _client(fake).post("/internal/news/sync?limit=0").status_code == 422
    assert fake.calls == []
...
def test_runner_is_wired_without_any_api_key():
    ...
    assert cron.get_sync_runner() is cron.run_news_sync
