...
class _FakeRunner:
    ...
    def __call__(self, limit: int | None = None) -> RecommendationsSyncReport:
...
def test_accepts_the_trigger_and_runs_the_sweep_with_the_limit():
    fake = _FakeRunner(RecommendationsSyncReport(refreshed=7, failed=2, limit=50))
...
def test_a_trigger_while_a_sweep_runs_is_a_noop():
    fake = _FakeRunner(RecommendationsSyncReport(0, 0, 1))
...
def test_runner_is_wired_without_any_api_key():
    ...
    assert cron.get_sync_runner() is cron.run_recommendations_sync
