...
from app.stocks.endpoints.cron import insider_transactions_endpoints as cron
from app.stocks.company.insider_transactions.use_cases import InsiderTransactionsSyncReport


class _FakeRunner:
    def __init__(self, report: InsiderTransactionsSyncReport) -> None:
        self._report = report
        self.calls: list[int | None] = []

    def __call__(self, limit: int | None = None) -> InsiderTransactionsSyncReport:
...
def test_accepts_the_trigger_and_runs_the_sweep_with_the_limit():
    fake = _FakeRunner(InsiderTransactionsSyncReport(refreshed=7, failed=2, limit=50))
    resp = _client(fake).post("/internal/insider-transactions/sync?limit=50")
...
def test_defaults_to_unlimited_when_omitted():
    fake = _FakeRunner(InsiderTransactionsSyncReport(refreshed=0, failed=0, limit=None))
    resp = _client(fake).post("/internal/insider-transactions/sync")
    assert resp.status_code == 202
...
def test_a_trigger_while_a_sweep_runs_is_a_noop():
    fake = _FakeRunner(InsiderTransactionsSyncReport(0, 0, 1))
...
def test_rejects_an_out_of_range_limit():
    fake = _FakeRunner(InsiderTransactionsSyncReport(0, 0, 1))
    assert _client(fake).post("/internal/insider-transactions/sync?limit=0").status_code == 422
    assert fake.calls == []
    assert cron._sync_lock.acquire(blocking=False)
    cron._sync_lock.release()
...
def test_runner_is_wired_without_any_api_key():
    ...
    assert cron.get_sync_runner() is cron.run_insider_transactions_sync
